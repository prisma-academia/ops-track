...
const Schema = z.object({
  ...
  litersRequested: z.coerce.number().min(0).default(0),
}).superRefine((data, ctx) => {
  if (data.sendInvitation) {
    if (!data.transporterId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Transporter is required when sending an invitation",
        path: ["transporterId"],
      });
    }
    if (!data.destination) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Destination is required when sending an invitation",
        path: ["destination"],
      });
    }
    if (!data.litersRequested || data.litersRequested <= 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Requested volume must be greater than 0",
        path: ["litersRequested"],
      });
    }
  }
});
