...
export function UserDetailActions({
  ...
  const fleetModulesList = modulesList.filter((m) =>
    m.perms.some((p) => isFleetPermissionKey(p.key))
  );
  const stationModulesList = modulesList.filter(
    (m) =>
      m.moduleName.startsWith("tenant.") &&
      !m.perms.some((p) => isFleetPermissionKey(p.key) || isMobilePermissionKey(p.key))
  );
  ...
  return (
    <div className="space-y-6">
      {isTenant ? (
        <>
          {(isEditMode || !hideEditTrigger) && (
            <div className="flex justify-end">{editorControls}</div>
          )}
          {renderPermissionsCard(
            fleetModulesList,
            "Fleet Web Portal Permissions",
            "Access control for fleet operations on the web admin dashboard."
          )}
          {renderPermissionsCard(
            stationModulesList,
            "Station Web Portal Permissions",
            "Access control for station operations on the web admin dashboard."
          )}
          {renderPermissionsCard(
            mobileModulesList,
        ...
        <>
          {renderPermissionsCard(
            ...
            "Access control for the web admin dashboard.",
            true
          )}
          {renderPermissionsCard(
            mobileModulesList,
        ...
        <div
          className={`p-3 text-sm rounded-md border ${
            info
              ...
              : "bg-red-50/50 border-red-200 text-red-600 dark:bg-red-900/20 dark:border-red-900 dark:text-red-400"
          }`}
        >
          {info || error}
        </div>
        ...
        <Card className="border-red-200/50 dark:border-red-900/50 shadow-sm">
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground mb-4">
              Resetting the password will immediately revoke all active sessions for this user and email them a recovery
              link.
            </p>
            <Button
              variant="destructive"
