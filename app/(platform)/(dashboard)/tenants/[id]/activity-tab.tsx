...
import type { ActivityRow } from "@/app/admin/(fleet)/activity/activity-table";
...
export function TenantActivityTab({
  ...
  const { data, meta, isLoading, setPage, setPageSize, setInitialData, refresh } =
    usePaginatedQuery<ActivityRow>({
      baseUrl: `/api/platform/tenants/${tenantId}/activity-logs`,
      additionalParams: {
  ...
  }, [date, action, status, debouncedDate, debouncedAction, debouncedStatus, setPage]);
  ...
  const rows =
    data.length > 0 || debouncedDate || debouncedAction || debouncedStatus
      ? data
      : initialData;

  const stats: ActivityStats = (meta as { stats?: ActivityStats }).stats ?? {
    total: (meta.totalCount as number) ?? initialData.length,
    success: initialData.filter(
      (r) => (r.status ?? getActivityStatus(r.action)) === "SUCCESS",
    ).length,
    failed: initialData.filter(
      (r) => (r.status ?? getActivityStatus(r.action)) === "FAILED",
    ).length,
  ...
  const isFiltered = Boolean(date || action || status);
  ...
  const columns = React.useMemo<ColumnDef<ActivityRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const value = (
            row.original.status ?? getActivityStatus(row.original.action)
          ) as ActivityStatus;
          return (
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border/40 bg-card p-4">
        ...
        <DataTable
          columns={columns}
          data={rows}
          tableId="tenant-activity"
          filterFields={filterFields}
          hideToolbar
          isLoading={isLoading}
          emptyMessage="No activity logs found for this tenant."
          onRefresh={refresh}
          serverPagination={{
            page: meta.page,
